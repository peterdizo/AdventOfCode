...
def make_step(dir: Char, pos: (Int, Int), path: List[(Char, Int)]) : (Int, Int) = {
  if (path.isEmpty) pos
  else {
    val step = path.head
    val new_direction = next_orientation(dir, step._1)
    val new_pos = move(new_direction, step._2, pos)
    println(new_direction + " " + new_pos)
    make_step(new_direction, new_pos, path.tail)
  }
}

println(make_step('N', (0,0), steps_tuples))